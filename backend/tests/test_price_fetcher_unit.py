...
We stub Yahoo Finance, proxy estimation, and DB session to validate the tiered
logic (yahoo -> proxy -> interpolation) and price interpolation behavior.
"""
...
def test_tier1_uses_yahoo_when_available(monkeypatch, sample_dates):
    ...
    service.proxy_estimator.yahoo_fetcher = stub_fetcher

    # Force ticker mapping to succeed
    monkeypatch.setattr("app.services.price_fetcher.get_yahoo_ticker", lambda symbol: "VTI")
    monkeypatch.setattr("app.services.price_fetcher.get_proxy_info", lambda name: None)
...
def test_tier2_uses_proxy_when_yahoo_missing(monkeypatch, sample_dates):
    """When Yahoo mapping fails but proxy exists, service should estimate via proxy."""
    start_date, end_date = sample_dates
    # Proxy prices: simple upward trend for deterministic ratio
    proxy_df = pd.DataFrame({"price": [10, 20]}, index=pd.date_range(start_date, periods=2, freq="D"))

    service = HistoricalPriceService(db=FakeSession([]))
    stub_fetcher = StubFetcher({"SPY": proxy_df})
    service.yahoo_fetcher = stub_fetcher
    service.proxy_estimator.yahoo_fetcher = stub_fetcher

    # No direct ticker, but proxy mapping available
    monkeypatch.setattr("app.services.price_fetcher.get_yahoo_ticker", lambda symbol: None)
    monkeypatch.setattr(
        "app.services.price_fetcher.get_proxy_info",
        lambda name: {
            "proxy": "SPY",
            "name": "S&P 500",
            "currency": "USD",
    ...
    # Reference transaction to anchor NAV
    ref_tx = SimpleNamespace(transaction_date=start_date, amount_jpy=100, quantity=1)
    service.db = FakeSession([ref_tx])
    ...
    assert source == "proxy"
    assert prices is not None
    # First proxy price is 10, reference price 100 -> expected 100; next should double
    assert pytest.approx(prices.iloc[0]["price"], rel=1e-5) == 100
    assert pytest.approx(prices.iloc[-1]["price"], rel=1e-5) == 200


def test_tier3_interpolates_when_no_sources(monkeypatch):
    """Service should fall back to linear interpolation using transaction prices."""
    start_date = date(2024, 1, 1)
    end_date = date(2024, 1, 3)

    # Disable yahoo and proxy
    monkeypatch.setattr("app.services.price_fetcher.get_yahoo_ticker", lambda symbol: None)
    monkeypatch.setattr("app.services.price_fetcher.get_proxy_info", lambda name: None)
    ...
    service.proxy_estimator.yahoo_fetcher = service.yahoo_fetcher
